...
class SAN_decoder(nn.Module):
    ...
    def __init__(self, params):
        ...
        elif params['encoder']['net'] == 'StrokeAwareEncoder':
            self.ratio = 16  # Default ratio for stroke encoder
        else:
            self.ratio = 16 * params['resnet']['conv1_stride']
    ...
    def forward(self, cnn_features, labels, images_mask, labels_mask, is_train=True):
        ...
        # Keep original mask for init_hidden and all attention mechanisms
        images_mask_full = images_mask  # Full resolution mask for attention mechanisms
        images_mask_downsampled = images_mask[:, :, ::self.ratio, ::self.ratio]  # Downsampled only for KL loss
        ...
        if is_train:
            ...
            parent_hiddens[:batch_size, :] = self.init_hidden(cnn_features, images_mask_full)
            ...
            for i in range(num_steps):
                ...
                word_context_vec, word_alpha, word_alpha_sum = self.word_attention(cnn_features, word_hidden_first,
                                                                                   word_alpha_sum, images_mask_full)
                ...
                c2p_context_vec, c2p_alpha, c2p_alpha_sum = self.c2p_attention(cnn_features, c2p_hidden_first,
                                                                               c2p_alpha_sum, images_mask_full)
            ...
            struct_list = []
            parent_hidden = self.init_hidden(cnn_features, images_mask_full)
            for i in range(num_steps):
