#!/usr/bin/env python3
"""
Test inference on trained arrow expression model: A \to k(x)
"""
import torch
import os
import glob
...
def test_arrow_inference():
    print("=== Testing Arrow Expression Model Inference ===")
    
    config_path = "config_arrow_expression.yaml"
    ...
    # Load the latest checkpoint from arrow training
    checkpoint_dir = "checkpoints_arrow_expression"
    ...
    # Get the second latest checkpoint to avoid potential corruption
    checkpoints_sorted = sorted(checkpoints, key=lambda x: int(x.split('_')[-1].split('.')[0]))
    best_checkpoint = checkpoints_sorted[-2] if len(checkpoints_sorted) > 1 else checkpoints_sorted[-1]
    ...
    # Test the arrow expression sample
    ...
    print(f"\nExpected sequence (smart tokenized targets):")
    for pos in range(min(12, target_labels.shape[0])):
    ...
    with torch.no_grad():
        ...
        if predicted_text.strip() == sample['label'].strip():
            ...
            print("The SmartLaTeXConverter + SAN model correctly predicts arrow expression!")
            return True
        else:
            print("Different result - analyzing...")
            
            # Show confidence scores for critical positions
            print(f"\nConfidence analysis (first 10 positions):")
...
if __name__ == '__main__':
    success = test_arrow_inference()
    if success:
        print("\nArrow expression training SUCCESS!")
        print("The model handles A \\to k(x) correctly!")
    else:
