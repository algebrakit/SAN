#!/usr/bin/env python3
"""
Test inference on trained fraction model
"""
import torch
import os
import glob
...
def test_fraction_inference():
    print("=== Testing Fraction Model Inference ===")
    ...
    # Load the latest checkpoint from fraction training
    ...
    with torch.no_grad():
        ...
        if predicted_text.strip() == sample['label'].strip():
            ...
            print("The stroke-aware SAN model correctly predicts the fraction!")
            return True
        else:
            ...
            print(f"\\nConfidence analysis:")
            for pos in range(min(8, word_probs.shape[1])):
...
if __name__ == '__main__':
    success = test_fraction_inference()
    if success:
        print("\\nFraction model validation complete! Ready for complex expressions.")
    else:
