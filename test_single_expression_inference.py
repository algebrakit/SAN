#!/usr/bin/env python3
"""
Test inference on trained single expression model: a=(x,v)
"""
import torch
import os
import glob
...
def test_single_expression_inference():
    print("=== Testing Single Expression Model Inference ===")
    
    config_path = "config_single_expression.yaml"
    ...
    # Load the latest checkpoint from single expression training
    checkpoint_dir = "checkpoints_single_expression"
    ...
    # Test the single expression sample
    ...
    with torch.no_grad():
        ...
        if predicted_text.strip() == sample['label'].strip():
            ...
            print("The stroke-aware SAN model correctly predicts the expression!")
            return True
        else:
...
if __name__ == '__main__':
    success = test_single_expression_inference()
    if success:
        print("\nSingle expression model validation complete! Ready for complex expressions.")
    else:
