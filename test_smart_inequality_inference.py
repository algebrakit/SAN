#!/usr/bin/env python3
"""
Test inference on trained smart inequality model: a \\neq c \\neq b.
"""
import torch
import os
import glob
...
def test_smart_inequality_inference():
    print("=== Testing Smart Inequality Model Inference ===")
    
    config_path = "config_smart_inequality.yaml"
    ...
    # Load the latest checkpoint from smart inequality training
    checkpoint_dir = "checkpoints_smart_inequality"
    ...
    # Test the smart inequality sample
    ...
    with torch.no_grad():
        ...
        if predicted_text.strip() == sample['label'].strip():
            ...
            print("The SmartLaTeXConverter + SAN model correctly predicts inequality expression!")
            print("Fixed the LaTeX tokenization for complex expressions with repeated commands!")
            return True
        else:
...
if __name__ == '__main__':
    success = test_smart_inequality_inference()
    if success:
        print("\\nSmart LaTeX tokenization SUCCESS for complex expressions!")
        print("The model handles repeated LaTeX commands (\\neq) correctly!")
    else:
