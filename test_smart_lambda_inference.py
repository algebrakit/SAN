#!/usr/bin/env python3
"""
Test inference on trained smart lambda model: \lambda_{2(M)}
"""
import torch
import os
import glob
...
def test_smart_lambda_inference():
    print("=== Testing Smart Lambda Model Inference ===")
    
    config_path = "config_smart_lambda.yaml"
    ...
    # Load the latest checkpoint from smart lambda training
    checkpoint_dir = "checkpoints_smart_lambda"
    ...
    with torch.no_grad():
        ...
        if predicted_text.strip() == sample['label'].strip():
            ...
            print("The SmartLaTeXConverter + SAN model correctly predicts \\lambda_{2(M)}!")
            print("✅ Fixed the character-by-character parsing issue!")
            return True
        else:
            print("Still different - analyzing...")
...
if __name__ == '__main__':
    success = test_smart_lambda_inference()
    if success:
        print("\n🚀 Smart LaTeX tokenization SUCCESS!")
        print("The model now properly handles LaTeX commands as semantic units!")
    else:
