...
def load_config(yaml_path):
    ...
    # Check if this is stroke configuration (has inkml_folder) or image configuration
    is_stroke_config = 'inkml_folder' in params
    
    if is_stroke_config:
        # Stroke-based configuration validation
        if not params.get('inkml_folder'):
            print('inkml_folder cannot be empty for stroke training!')
            exit(-1)
        
        if not params.get('word_path'):
            print('word dict cannot be empty')
            exit(-1)
        
        print(f"Loaded stroke configuration with {len(params)} parameters")
    else:
        # Image-based configuration validation (original logic)
        if not params.get('train_image_path'):
            print('training images cannot be empty!')
            exit(-1)

        if not params.get('train_label_path'):
            print('training labels cannot be empty!')
            exit(-1)

        if not params.get('eval_image_path'):
            print('test images cannot be empty!')
            exit(-1)

        if not params.get('eval_label_path'):
        ...
        print(f"Loaded image configuration with {len(params)} parameters")
...
def save_checkpoint(model, optimizer, word_score, struct_score, ExpRate_score, epoch, optimizer_save=False, path='checkpoints', multi_gpu=False, local_rank=0):
    ...
    filename = f'{checkpoint_dir}/{model.name}_WordRate-{word_score:.4f}_structRate-{struct_score:.4f}_ExpRate-{ExpRate_score:.4f}_{epoch}.pth'
...
def cal_score(probs, labels, mask):
    ...
    batch_size = probs[0].shape[0]
    word_probs, struct_probs = probs
    word_label, struct_label = labels[:,:,0], labels[:,:,4:]
    struct_label = struct_label.contiguous().view(batch_size, -1)
    line_right = 0
    _, word_pred = word_probs.max(2)
